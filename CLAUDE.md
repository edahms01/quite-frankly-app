...
- **SubscriptionCheckout dismiss-vs-success** — `SubscriptionCheckout.js` treats any dismissal of the checkout browser sheet the same as a completed purchase (navigates to `SubscriptionConfirmed` either way). This is the standing Phase 6 design decision (`TRUST_ANY_CLOSE_AS_COMPLETE`) — checkout provider (Squarespace) gives no redirect signal to distinguish "completed" from "backed out," and `SubscriptionConfirmed` has never been gated behind real membership verification. Deferred alongside "Member-unlocked states" since real verification would eventually resolve both.
