...
- **Quite Frankly (native)**: load Frank's existing Squarespace checkout in a system-browser sheet (SFSafariViewController/Custom Tabs via `expo-web-browser`'s `openAuthSessionAsync`), not an embedded WebView — needed for autofill/security (see component-map's Subscription Checkout notes). It's already responsive/mobile-friendly — no separate "mobile version" to build.
